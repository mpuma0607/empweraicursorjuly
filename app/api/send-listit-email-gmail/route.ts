...
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 LISTIT EMAIL GMAIL API - Starting request')
    ...
    console.log('✅ ListIt email sent successfully via Gmail:', result.id)
    
    return NextResponse.json({ 
      success: true, 
      messageId: result.id,
      message: 'ListIt content sent successfully via Gmail'
    })

  } catch (error) {
    console.error('❌ Error sending listit email via Gmail:', error)
    return NextResponse.json(
