...
export default function IdeaHubBegginsForm() {
  ...
  useEffect(() => {
    async function loadUserBrandingProfile() {
      ...
      try {
        const profile = await getUserBrandingProfile(user.id.toString(), "century21-beggins")
  ...
  const saveToProfile = async () => {
    ...
    setIsSaving(true)
    try {
      const title = generateCreationTitle("ideahub-beggins", formData)
            ...
            {formData.wantBranding
              ? formData.brandingChoice === "saved-brand"
                ...
                : formData.brandingChoice === "saved-logo"
                  ...
                  : formData.brandingChoice === "dropdown"
                    ? `Selected: ${begginsBrandOptions.find((b) => b.value === formData.selectedBrand)?.label}`
                    : "Custom logo upload"
              : "None"}
          </p>
          <p>
