...
export default function MyMarketForm() {
  ...
  const formatMarketData = (data: any) => {
    ...
    try {
      ...
      // Handle rental market data - simplified and more reliable
      if (data.rental_market_trends || data.rental_data || data.rental_market || data.average_rent || data.avgRent) {
        const rentalData = data.rental_market_trends || data.rental_data || data.rental_market || data
      ...
      console.log("MyMarket AI: No recognized data structure found, using fallback")
      return {
        type: 'unknown',
        location: data.search_query || "Unknown Location",
        rawData: data
